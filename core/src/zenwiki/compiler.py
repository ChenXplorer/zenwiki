...
_LINT_GATE_BLOCKING: set[str] = {
    ...
    "thin_map",                 # map with < 5 members → reject (over-eager synthesis)
    "incomparable_subjects",    # comparison with < 2 subjects → reject
...
# Consolidate is skipped when the wiki has fewer total pages than this. Below
# the floor there is not enough breadth to find genuine cross-source overlap,
# and running the Agent anyway produces hallucinated maps/comparisons (e.g.
# a map of 2 concepts, a comparison of non-comparable subjects).
_CONSOLIDATE_MIN_PAGES = 5
...
def _run_consolidate(
    ...
    total_pages = sum(_wiki_stats(root).values())
    if total_pages < _CONSOLIDATE_MIN_PAGES:
        console.print(
            f"[dim]Skipping consolidate — wiki has {total_pages} page(s), "
            f"need >= {_CONSOLIDATE_MIN_PAGES} for meaningful synthesis.[/dim]"
        )
        return True  # not a failure; this is expected on thin wikis
