...
from .config import RAW_SECTIONS, WIKI_SECTIONS, find_project_root, load_config
from .markdown import slugify as _slugify

app = typer.Typer(help="ZenWiki — Agent-driven knowledge wiki toolkit.", no_args_is_help=True)
console = Console()
...
def init(
    ...
    """Create a new ZenWiki project with CLAUDE.md, config.yaml, raw/ and wiki/ directories."""
    root = path.resolve()
    root.mkdir(parents=True, exist_ok=True)

    for section in RAW_SECTIONS:
        (root / "raw" / section).mkdir(parents=True, exist_ok=True)

    for section in WIKI_SECTIONS:
        (root / "wiki" / section).mkdir(parents=True, exist_ok=True)

    (root / ".zenwiki").mkdir(parents=True, exist_ok=True)

    claude_md = root / "CLAUDE.md"
    if not claude_md.exists():
        claude_md.write_text(_read_template("CLAUDE.md"), encoding="utf-8")
        console.print(f"[green]✓[/green] Created {claude_md.relative_to(root)}")
    else:
        console.print(f"[yellow]⊘[/yellow] {claude_md.relative_to(root)} already exists, skipped")

    config_yaml = root / "config.yaml"
    if not config_yaml.exists():
        config_yaml.write_text(_read_template("config.yaml"), encoding="utf-8")
        console.print(f"[green]✓[/green] Created {config_yaml.relative_to(root)}")

    gitignore = root / ".gitignore"
    if not gitignore.exists():
        gitignore.write_text(_read_template("gitignore"), encoding="utf-8")
        console.print(f"[green]✓[/green] Created {gitignore.relative_to(root)}")

    index_md = root / "wiki" / "index.md"
    if not index_md.exists():
        index_md.write_text("# ZenWiki Index\n\n_No pages yet. Run `/ingest` to add your first source._\n", encoding="utf-8")

    log_md = root / "wiki" / "log.md"
    if not log_md.exists():
        log_md.write_text("# ZenWiki Log\n\n", encoding="utf-8")

    console.print(f"\n[bold green]ZenWiki initialized at {root}[/bold green]")
    console.print("Next steps:")
    console.print("  1. Put source files into raw/")
    console.print("  2. Run: [bold]zenwiki compile[/bold]          (auto-compile via Agent CLI)")
    console.print("     -- or --")
    console.print("  2. Open this directory in Claude Code / Codex CLI")
    console.print("  3. Ask the agent: /ingest")
    console.print("")
    console.print("  Run: [bold]zenwiki doctor[/bold]              (check environment readiness)")
...
def doctor(
    ...
    """Check environment readiness: config, directories, tools."""
    root = _root(path)
    ok_count = 0
    fail_count = 0
    ...
    _check((root / "config.yaml").exists(), "config.yaml")
    _check((root / "CLAUDE.md").exists(), "CLAUDE.md")
    _check((root / ".zenwiki").is_dir(), ".zenwiki/ directory")

    raw_exists = (root / "raw").is_dir()
    _check(raw_exists, "raw/ directory")
    if raw_exists:
        for section in ("papers", "articles", "notes", "docs"):
            _check((root / "raw" / section).is_dir(), f"  raw/{section}/")

    wiki_exists = (root / "wiki").is_dir()
    _check(wiki_exists, "wiki/ directory")

    claude_path = shutil.which("claude")
    _check(claude_path is not None, "claude CLI", claude_path or "not found")

    codex_path = shutil.which("codex")
    _check(codex_path is not None, "codex CLI", codex_path or "not found")

    qmd_path = shutil.which("qmd")
    _check(qmd_path is not None, "qmd CLI (vector search)", qmd_path or "not found — hybrid search disabled")

    _check((root / ".git").is_dir(), "git initialized")
